...
const mp = (() => {
  ...
  function init() {
    ...
    persons = Array.from({length:4}, () => ({
      ...
      count: 4,
      cachedRotC: null  // cached current working image (post-crop) — reused during crop drag to avoid per-frame re-render on slow PCs
    }));
  ...
  function renderPersonCanvas(pi) {
    ...
    // BUGFIX (image stretches when cropping a second time): this used to cache
    // `rotC` — the FULL pre-crop rotated image — for reuse during the live crop
    // drag redraw. But once a crop has already been applied, the crop canvas
    // (cv.width/cv.height below) is sized to the CROPPED image's aspect ratio,
    // not the full original. Drawing the full-size rotC into that
    // differently-shaped canvas during the next crop drag squeezed/stretched it.
    // Caching the actual current working image (post-crop, post-brightness —
    // i.e. exactly what's on screen) keeps the aspect ratio correct on every
    // re-crop, and is also cheaper (smaller canvas) on low-end PCs.
    p.cachedRotC = cropSrc; // cache for use during crop drag on slow PCs
